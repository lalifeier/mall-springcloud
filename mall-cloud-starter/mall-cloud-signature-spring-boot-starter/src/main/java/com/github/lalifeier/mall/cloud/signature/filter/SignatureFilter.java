...
public class SignatureFilter extends OncePerRequestFilter {
    ...
    public SignatureFilter(
            RedisTemplate<String, Object> redisTemplate, SignatureProperties properties) {
    ...
    protected void doFilterInternal(
        ...
        if (StringUtils.isBlank(timestamp)
                || Long.parseLong(timestamp) + (1000 * NONCE_STR_TIMEOUT_SECONDS)
        ...
        if (StringUtils.isBlank(signature)
                || !StringUtils.equals(
                        signature, SignatureUtil.signature(requestPayload, requestWrapper))) {
