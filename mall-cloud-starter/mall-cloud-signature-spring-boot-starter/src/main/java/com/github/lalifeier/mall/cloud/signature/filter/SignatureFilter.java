...
public class SignatureFilter extends OncePerRequestFilter {
  ...
  public SignatureFilter(RedisTemplate<String, Object> redisTemplate,
  ...
  protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
    ...
    if (StringUtils.isBlank(timestamp) || Long.parseLong(timestamp)
        + (1000 * NONCE_STR_TIMEOUT_SECONDS) > System.currentTimeMillis()) {
    ...
    String requestPayload = IOUtils.toString(requestWrapper.getContentAsByteArray(),
        StandardCharsets.UTF_8.displayName());

    // 对请求头参数进行签名
    if (StringUtils.isBlank(signature) || !StringUtils.equals(signature,
        SignatureUtil.signature(requestPayload, requestWrapper))) {
