...
public class RedisRateLimiter {
  ...
  public boolean acquire(String key, RateLimiter rateLimiter) {
    ...
    RateLimiterAlgorithm rateLimiterAlgorithm =
        RateLimiterAlgorithmFactory.getRateLimiterAlgorithm(algorithmType.getName());
    RedisScript<List<Long>> script = rateLimiterAlgorithm.getScript();
    List<String> keys = rateLimiterAlgorithm.getKeys(key);
    List<String> scriptArgs = Arrays.asList(String.valueOf(rate), String.valueOf(capacity),
        String.valueOf(System.currentTimeMillis() / 1000), String.valueOf(perSecond));
