...
public class IntegerToEnumConverterFactory implements ConverterFactory<Integer, BaseEnum<Integer>> {
    private final ConcurrentMap<Class<? extends BaseEnum<Integer>>, IntegerToEnumConverter<?>>
            CONVERTERS = new ConcurrentHashMap<>();

    @Override
    public <T extends BaseEnum<Integer>> Converter<Integer, T> getConverter(
            @NotNull Class<T> targetType) {
        @SuppressWarnings("unchecked")
        IntegerToEnumConverter<T> converter =
                (IntegerToEnumConverter<T>) CONVERTERS.get(targetType);
        if (converter == null) {
