...
public class MultilevelCacheAutoConfiguration {
    ...
    @Resource private MultilevelCacheProperties multilevelCacheProperties;
    ...
    public RedisCache redisCache(
        ...
        RedisCacheConfiguration redisCacheConfiguration =
                defaultCacheConfig()
                        .entryTtl(
                                Duration.of(
                                        multilevelCacheProperties.getRedisExpireTime(),
                                        ChronoUnit.SECONDS))
                        .serializeKeysWith(
        ...
        RedisCache redisCache =
                new CustomizedRedisCache(
                        multilevelCacheProperties.getRedisName(),
                        redisCacheWriter,
    ...
    public CaffeineCache caffeineCache() {
        ...
        int maxCapacity =
                (int)
                        (Runtime.getRuntime().totalMemory()
                                * multilevelCacheProperties.getMaxCapacityRate());
        // 根据初始容量比率计算初始容量
        ...
        Caffeine<Object, Object> caffeineBuilder =
                Caffeine.newBuilder()
                        ...
                        .expireAfterAccess(
                                Duration.of(
                                        multilevelCacheProperties.getCaffeineExpireTime(),
                                        ChronoUnit.SECONDS)) // 设置缓存读取过期时间
                        .recordStats(); // 开启缓存统计信息记录

        // 创建并返回CaffeineCache对象
        CaffeineCache caffeineCache =
                new CaffeineCache(
                        multilevelCacheProperties.getCaffeineName(), caffeineBuilder.build());
