...
public class MultilevelCacheAutoConfiguration {
  ...
  @Bean
  public RedisCache redisCache(RedisConnectionFactory redisConnectionFactory, MultilevelCacheProperties multilevelCacheProperties) {
    ...
    RedisCache redisCache = new CustomizedRedisCache(multilevelCacheProperties.getRedisName(), redisCacheWriter, redisCacheConfiguration);
...
//      .scheduler(Scheduler.systemScheduler()) // 设置定时任务执行过期清除操作
      ...
      .expireAfterAccess(Duration.of(multilevelCacheProperties.getCaffeineExpireTime(), ChronoUnit.SECONDS)) // 设置缓存读取过期时间
      .recordStats(); // 开启缓存统计信息记录

    // 创建并返回CaffeineCache对象
    CaffeineCache caffeineCache = new CaffeineCache(multilevelCacheProperties.getCaffeineName(), caffeineBuilder.build());
    return caffeineCache;
  }

  @Bean
  @ConditionalOnBean({CaffeineCache.class, RedisCache.class})
  public MultilevelCache multilevelCache(RedisCache redisCache, CaffeineCache caffeineCache, MessagePublisher redisMessagePublisher) {
    MultilevelCache multilevelCache = new MultilevelCache(true, multilevelCacheProperties.getEnableFirstCache(), multilevelCacheProperties.getName(), redisCache, caffeineCache, redisMessagePublisher);
