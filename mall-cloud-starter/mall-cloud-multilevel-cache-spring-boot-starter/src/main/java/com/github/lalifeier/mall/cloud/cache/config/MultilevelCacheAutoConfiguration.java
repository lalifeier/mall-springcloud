...
public class MultilevelCacheAutoConfiguration {
  ...
  public RedisCache redisCache(RedisConnectionFactory redisConnectionFactory,
    ...
    RedisCache redisCache = new CustomizedRedisCache(multilevelCacheProperties.getRedisName(),
  ...
  @ConditionalOnProperty(name = "multilevel.cache.caffeineSwitch", havingValue = "true",
      matchIfMissing = true)
  public CaffeineCache caffeineCache() {
    // 根据最大容量比率计算最大容量
    int maxCapacity =
        (int) (Runtime.getRuntime().totalMemory() * multilevelCacheProperties.getMaxCapacityRate());
    // 根据初始容量比率计算初始容量
    int initCapacity = (int) (maxCapacity * multilevelCacheProperties.getInitRate());

    // 创建缓存构建器对象
    Caffeine<Object, Object> caffeineBuilder = Caffeine.newBuilder().initialCapacity(initCapacity) // 设置初始容量
        ...
        .expireAfterAccess(
            Duration.of(multilevelCacheProperties.getCaffeineExpireTime(), ChronoUnit.SECONDS)) // 设置缓存读取过期时间
  ...
  public MultilevelCache multilevelCache(RedisCache redisCache, CaffeineCache caffeineCache,
    ...
    MultilevelCache multilevelCache =
        new MultilevelCache(true, multilevelCacheProperties.getEnableFirstCache(),
            multilevelCacheProperties.getName(), redisCache, caffeineCache, redisMessagePublisher);
  ...
  public RedisMessageListenerContainer redisMessageListenerContainer(
    ...
    // 添加消息监听器和频道
    redisMessageListenerContainer.addMessageListener(redisCacheMessageListener,
        new ChannelTopic(multilevelCacheProperties.getTopic()));
