...
public class QueryWrapperUtils {
     ...
     */
    private static void addColumnsToWrapper(
            QueryWrapper<?> wrapper, Class<?> clazz, String[] columns) {
     ...
     */
    private static void addFiltersToWrapper(
            QueryWrapper<?> wrapper, Class<?> clazz, Map<String, String> filter) {
                ...
                .filter(
                        entry ->
                                StringUtils.isNotBlank(entry.getKey())
                                        && StringUtils.isNotBlank(entry.getValue())) // 过滤掉空白条件。
                .forEach(
                        entry ->
                                addFilterToWrapper(
                                        wrapper,
                                        clazz,
                                        entry.getKey(),
                                        entry.getValue())); // 添加到 QueryWrapper
        // 对象中。
    }
     ...
     * @param value 过滤条件的值。
     */
    private static void addFilterToWrapper(
            QueryWrapper<?> wrapper, Class<?> clazz, String key, String value) {
        ...
        if ("range".equals(operator) && key.endsWith("_range")) {
            ...
            if (method != null) {
                ReflectionUtils.invokeMethod(
                        method, wrapper, true, fieldName, typedValue); // 否则，使用对应的方法。
            }
        }
    }
                ...
                Stream.of(orderBy.split(","))
                        .map(
                                order -> {
                                    ...
                                    if (field != null) {
                                        return (Function<QueryWrapper<?>, QueryWrapper<?>>)
                                                (wrapper1 ->
                                                        wrapper1.orderBy(
                                                                true, isAsc, field.getName()));
                                    }
                                    return null;
                                })
