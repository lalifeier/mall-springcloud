...
public class QueryWrapperUtils {
   ...
   */
  private static void addColumnsToWrapper(QueryWrapper<?> wrapper, Class<?> clazz,
      String[] columns) {
    ...
    Stream.of(columns).filter(StringUtils::isNotBlank) // 过滤掉空白列。
   ...
   */
  private static void addFiltersToWrapper(QueryWrapper<?> wrapper, Class<?> clazz,
    ...
    filter.entrySet().stream().filter(
        entry -> StringUtils.isNotBlank(entry.getKey()) && StringUtils.isNotBlank(entry.getValue())) // 过滤掉空白条件。
        .forEach(entry -> addFilterToWrapper(wrapper, clazz, entry.getKey(), entry.getValue())); // 添加到
                                                                                                 // QueryWrapper
    // 对象中。
  }
   ...
   * @param value 过滤条件的值。
   */
  private static void addFilterToWrapper(QueryWrapper<?> wrapper, Class<?> clazz, String key,
      String value) {
    ...
    if ("range".equals(operator) && key.endsWith("_range")) {
      ...
      Method method = ReflectionUtils.findMethod(QueryWrapper.class, operator, boolean.class,
  ...
  public static void addSortToWrapper(QueryWrapper<?> wrapper, Class<?> clazz, String orderBy) {
    ...
    // 按逗号分隔排序选项，构造排序条件
    List<Function<QueryWrapper<?>, QueryWrapper<?>>> orderBys =
        Stream.of(orderBy.split(",")).map(order -> {
          ...
          if (field != null) {
            return (Function<QueryWrapper<?>, QueryWrapper<?>>) (wrapper1 -> wrapper1.orderBy(true,
                isAsc, field.getName()));
          }
          return null;
