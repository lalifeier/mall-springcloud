...
public class QueryWrapperUtils {
    ...
    private static void addFiltersToWrapper(QueryWrapper<?> wrapper, Class<?> clazz, Map<String, String> filter) {
        ...
        filter.entrySet().stream()
                .filter(entry ->
                        StringUtils.isNotBlank(entry.getKey()) && StringUtils.isNotBlank(entry.getValue())) // 过滤掉空白条件。
    ...
    private static void addFilterToWrapper(QueryWrapper<?> wrapper, Class<?> clazz, String key, String value) {
        ...
        if ("range".equals(operator) && key.endsWith("_range")) {
            ...
            Method method =
                    ReflectionUtils.findMethod(QueryWrapper.class, operator, boolean.class, Object.class, Object.class);
            if (method != null) {
                ...
                .map(order -> {
                    ...
                    if (field != null) {
                        return (Function<QueryWrapper<?>, QueryWrapper<?>>)
                                (wrapper1 -> wrapper1.orderBy(true, isAsc, field.getName()));
                    }
                    return null;
                })
