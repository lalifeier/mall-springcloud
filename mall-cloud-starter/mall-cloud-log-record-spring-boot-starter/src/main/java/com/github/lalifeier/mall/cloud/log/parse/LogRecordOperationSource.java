...
public class LogRecordOperationSource {
  ...
  private LogRecord parse(AnnotatedElement ae, OperationLog annotation) {
    LogRecord logRecord = LogRecord.builder().bizNo(annotation.bizNo()).type(annotation.type())
        .subType(annotation.subType()).success(annotation.success()).fail(annotation.fail())
        .extra(annotation.extra()).operatorId(annotation.operatorId())
        .operatorName(annotation.operatorName()).condition(annotation.condition()).build();
    validateLogRecordOperation(ae, logRecord);
    return logRecord;
  }

  private void validateLogRecordOperation(AnnotatedElement ae, LogRecord logRecord) {
    if (!StringUtils.hasText(logRecord.getSuccess()) && !StringUtils.hasText(logRecord.getFail())) {
      throw new IllegalStateException("Invalid logRecord annotation configuration on '"
          + ae.toString() + "'. 'one of success and fail' attribute must be set.");
    }
  }
}
