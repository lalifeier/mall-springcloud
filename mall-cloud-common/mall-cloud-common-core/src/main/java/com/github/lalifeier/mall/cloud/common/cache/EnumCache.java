package com.github.lalifeier.mall.cloud.common.cache;
...
public class EnumCache {
   ...
   * @param <E>
   */
  public static <E extends Enum> void registerByValue(Class<E> clazz, E[] es,
      EnumMapping<E> enumMapping) {
    ...
    for (E e : es) {
      ...
      if (map.containsKey(value)) {
        throw new RuntimeException(String.format("枚举%s存在相同的值%s映射同一个枚举%s.%s", clazz.getSimpleName(),
            value, clazz.getSimpleName(), e));
      }
      map.put(value, e);
    }
  ...
  private static <E extends Enum> E find(Class<E> clazz, Object obj,
    ...
    if (map == null) {
      ...
      if (cache == CACHE_BY_NAME) {
        msg = String.format(
            "枚举%s还没有注册到枚举缓存中，请在%s.static代码块中加入如下代码 :"
                + " EnumCache.registerByName(%s.class, %s.values());",
      ...
      if (cache == CACHE_BY_VALUE) {
        msg = String.format(
            "枚举%s还没有注册到枚举缓存中，请在%s.static代码块中加入如下代码 :"
                + " EnumCache.registerByValue(%s.class, %s.values()," + " %s::getXxx);",
            clazz.getSimpleName(), clazz.getSimpleName(), clazz.getSimpleName(),
