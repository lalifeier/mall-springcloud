...
public class DESUtil {
   ...
   * @param data            待加密数据
   * @param key             密钥对象
   * @param cipherAlgorithm 加密算法枚举类型，可以是DES_ECB_PKCS5Padding、DES_CBC_PKCS5Padding、DESede_ECB_PKCS5Padding或DESede_CBC_PKCS5Padding
   * @param iv              初始化向量，只有在使用CBC算法时才需要，可以为null
   * @return 加密后的数据，使用Base64编码
   * @throws Exception 如果加密过程出错
   */
  public static String encrypt(String data, SecretKey key, CipherAlgorithm cipherAlgorithm, byte[] iv) throws Exception {
   ...
   */
  public static String decrypt(String encryptedData, SecretKey key, CipherAlgorithm cipherAlgorithm, byte[] iv) throws Exception {
    Cipher cipher = Cipher.getInstance(cipherAlgorithm.getAlgorithm());
    if (cipherAlgorithm.getAlgorithm().contains("CBC")) {
      IvParameterSpec ivParameterSpec = new IvParameterSpec(iv);
