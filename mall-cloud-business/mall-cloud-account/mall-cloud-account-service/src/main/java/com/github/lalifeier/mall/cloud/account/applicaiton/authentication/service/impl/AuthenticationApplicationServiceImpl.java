...
public class AuthenticationApplicationServiceImpl implements AuthenticationApplicationService {
  ...
  public RegisterDTO register(RegisterCommand command) {
    Long count = accountRepository.countByUserNameOrEmailOrPhone(new AccountName(command.getUsername()), new Email(command.getEmail()), new PhoneNumber(command.getPhone()));
    if (count > 0) {
  ...
  public LoginDTO login(LoginCommand command) {
    return loginProviders.stream()
      ...
      .orElseThrow(
        () ->
          new LoginException(LoginErrorCodeEnum.B_LOGIN_TYPE_NOT_SUPPORT));
  }
}
