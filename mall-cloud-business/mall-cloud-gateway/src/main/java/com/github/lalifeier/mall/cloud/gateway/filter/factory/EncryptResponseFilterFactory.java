...
@Slf4j
@Component
public class EncryptResponseFilterFactory
    extends AbstractGatewayFilterFactory<EncryptResponseFilterFactory.Config> {
  ...
  public class EncryptResponseFilter implements GatewayFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
      ...
      return chain.filter(exchange.mutate()
          .response(new EncryptResponseServerHttpResponse(exchange, this.config)).build());
    }
  ...
  protected static class EncryptResponseServerHttpResponse extends ServerHttpResponseDecorator {
    ...
    public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
      ...
      Mono<byte[]> data = bufferFlux.collectList().map(dataBuffers -> {
        byte[] array = new byte[dataBuffers.stream().mapToInt(DataBuffer::readableByteCount).sum()];
        int position = 0;
      ...
      return data.flatMap(bytes -> {
        ...
        DataBuffer buffer =
            this.bufferFactory().wrap(errorMessage.getBytes(StandardCharsets.UTF_8));
